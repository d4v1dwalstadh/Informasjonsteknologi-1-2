...
def finn_elev(elev_navn, skoledag, klokke):
    try:
        elev = None
        ...
        if time_tider["forste_time"]["start"] <= klokkeslett <= time_tider["forste_time"]["slutt"]:
            print(f"{elev_navn} er i klasserom: {klasserom_oversikt[timeplan[f"{dag}"][0]]}")
            
        elif time_tider["andre_time"]["start"] <= klokkeslett <= time_tider["andre_time"]["slutt"]:
            print(f"{elev_navn} i klasserom: {klasserom_oversikt[timeplan[f"{dag}"][1]]}")
            
        elif time_tider["tredje_time"]["start"] <= klokkeslett <= time_tider["tredje_time"]["slutt"]:
            print(f"{elev_navn} er i klasserom: {klasserom_oversikt[timeplan[f"{dag}"][2]]}")
            
        elif time_tider["fjerde_time"]["start"] <= klokkeslett <= time_tider["fjerde_time"]["slutt"]:
            print(f"{elev_navn} er i klasserom: {klasserom_oversikt[timeplan[f"{dag}"][3]]}")

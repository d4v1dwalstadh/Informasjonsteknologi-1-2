...
timeplan = {
    "mandag": [fag1, fag2, fag3, klasse],
    "tirsdag": [klasse, fag1, fag2, fag3],
    "onsdag": [fag1, fag2, fag3, klasse],
    "torsdag": [klasse, fag1, fag2, fag3],
    "fredag": [fag1, klasse]
}
...
def finn_elev(elev_navn, dag, klokke):
    try:
        elev = elever.get(elev_navn)
        if elev is None:
            print(f"Eleven {elev_navn} ble ikke funnet i systemet.")

        timeplan_dag = timeplan.get(dag)
        if timeplan_dag is None:
            print(f"Ugyldig dag: {dag}. Vennligst skriv en gyldig ukedag (Mandag, Tirsdag, osv).")

        for time_bolk, fagliste in timeplan_dag.items():
            if klokkeslett_i_range(klokke, time_tider[time_bolk]):
                print(f"{elev_navn} er i klasserom for {fagliste} i timebolken {time_bolk}.")

        print(f"{elev_navn} har enten friminutt eller er ferdig på skolen.")
    except ValueError:
        print("Ugyldig klokkeslettformat. Bruk HH:MM.")


def klokkeslett_i_range(klokke, tidsintervall):
    klokkeslett = datetime.strptime(klokke, "%H:%M")
    return tidsintervall["start"] <= klokkeslett <= tidsintervall["slutt"]

finn_elev("Mads Hansen", "Mandag", "09:00")
